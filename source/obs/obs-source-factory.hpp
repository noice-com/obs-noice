...
namespace noice::obs {
template<class _factory, typename _instance> class source_factory {
protected:
...
private /* Factory */:
	static const char *_get_name(void *type_data) noexcept
	{
		try {
			...
			DLOG_ERROR("Unexpected exception in function '%s': %s.", __FUNCTION__, ex.what());
			return nullptr;
		} catch (...) {
			DLOG_ERROR("Unexpected exception in function '%s'.", __FUNCTION__);
			return nullptr;
		}
	}
...
public /* Instance > Audio */:
	...
	static bool _audio_render(void *data, uint64_t *ts_out, struct obs_source_audio_mix *audio_output, uint32_t mixers,
	...
	static bool _audio_mix(void *data, uint64_t *ts_out, struct audio_output_data *audio_output, std::size_t channels,
...
public /* Instance > Configuration */:
	static void _load(void *data, obs_data_t *settings) noexcept
	{
		try {
			...
			if (priv) {
				uint64_t version = static_cast<uint64_t>(obs_data_get_int(settings, "version"));
				priv->migrate(settings, version);
				obs_data_set_int(settings, "version", static_cast<int64_t>(PROJECT_VERSION_INT));
				priv->load(settings);
			}
		} catch (const std::exception &ex) {
			DLOG_ERROR("Unexpected exception in function '%s': %s.", __FUNCTION__, ex.what());
...
public:
	virtual const char *get_name() { return "Not Yet Implemented"; }
	...
	virtual obs_properties_t *get_properties2(_instance *data) { return nullptr; }
};

class source_instance {
protected:
	::noice::obs::source _self;

public:
	source_instance(obs_data_t *settings, obs_source_t *source) : _self(source, false, false) {}
	virtual ~source_instance(){};

	virtual ::noice::obs::source get() { return _self; }
...
public /* Instance > Video */:
	...
	virtual struct obs_source_frame *filter_video(struct obs_source_frame *frame) { return frame; }

public /* Instance > Audio */:
	virtual struct obs_audio_data *filter_audio(struct obs_audio_data *audio) { return audio; }

	virtual bool audio_render(uint64_t *ts_out, struct obs_source_audio_mix *audio_output, uint32_t mixers, std::size_t channels,
...
public /* Media Controls */:
	...
	virtual int64_t media_get_duration() { return 0; }

	virtual int64_t media_get_time() { return 0; }

	virtual void media_set_time(int64_t milliseconds) {}

	virtual obs_media_state media_get_state() { return OBS_MEDIA_STATE_NONE; }

public /* Missing Files */:
	virtual obs_missing_files_t *missing_files() { return nullptr; };
};
