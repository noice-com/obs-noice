...
function Build {
    ...
    $BuildSpecFile = if ($env:BUILDSPEC_FILE) { "${ProjectRoot}/${env:BUILDSPEC_FILE}" } else { "${ProjectRoot}/buildspec.json" };
    ...
    if ( ! ( ( $SkipAll ) -or ( $SkipBuild ) ) ) {
        ...
        Invoke-External cmake -S . -B build_${script:Target} --install-prefix "${ProjectRoot}/release" @CmakeArgs
